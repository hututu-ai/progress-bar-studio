...
完成并保存过一次方案后，下一条视频的第一个问题会直接显示上次确认的
**角色、精确样式、配色、大小和位置**。选择“整套沿用”后，这些内容不会
再逐项询问；ElleFlow 只重新分析新视频的章节，并继续做真实画面适配和导出检查。

章节数量不会跟着预设固定下来。3 分钟内通常建议 3–5 段；内容转折确实较多
且标签仍清楚时可以到 6–7 段，长视频则根据内容密度增加，不按时长平均切分。
...
| --- | --- | --- |
| 1. 复用与章节 | “沿用上次方案吗？这些章节对吗？” | 可整套沿用视觉配置，再确认新视频章节 |
...
可以。ElleFlow 会先询问是否保存预设，最终方案确认后把角色、精确样式、
配色、大小和位置一起保存到本地。下次会在第一步问你是否整套沿用；确认后
不再重复询问这些字段。视频、章节、输出文件夹和导出授权不会被保存。
